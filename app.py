...
def index():
    ...
    if request.method == 'POST':
        ...
        asics_unit_value = float(request.form.get('asics_unit_value', 0))
        shelter = float(request.form.get('shelter', 0))
        generator = float(request.form.get('generator', 0))
        infrastructure = float(request.form.get('infrastructure', 0))
        general_expenses = float(request.form.get('general_expenses', 0))
        other_capex_total = 0
        ...
        for i in range(1, 21):  # Assume up to 20 rows
            ...
            if model:
                units = int(request.form.get(f'asic_units_{i}', 0))
                price = float(request.form.get(f'asic_price_{i}', 0))
                hashrate = float(request.form.get(f'asic_hashrate_{i}', 0))
                consumption = float(request.form.get(f'asic_consumption_{i}', 0))
                asics.append({
                    'model': model,
                    'units': units,
                    'price': price,
        ...
        if power_source == 'Direct Energy':
            energy_cost_per_mwh = float(request.form.get('energy_cost_per_mwh', 0))
            energy_cost_per_kwh = energy_cost_per_mwh / 1000
        else:
            gas_price_per_mwh = float(request.form.get('gas_price_per_mwh', 0))
        ...
        financing_rate = float(request.form.get('financing_rate', 0))
        tax_rate = float(request.form.get('tax_rate', 0))
        downtime_percent = float(request.form.get('downtime_percent', 0))
        asic_efficiency = float(request.form.get('asic_efficiency', 30))
        btc_price_override = request.form.get('btc_price_override')
        depreciation_years = int(request.form.get('depreciation_years', 3))
        depreciation_method = request.form.get('depreciation_method', 'linear')
        operational_costs = float(request.form.get('operational_costs', 0))
        ...
        # Run simulation
        results = simulate_mining(investment_usd, hashrate_th, energy_cost_per_kwh, btc_price, difficulty, asic_efficiency, depreciation_years, depreciation_method, operational_costs, downtime_percent)
